...
def block35(net, scale=1.0, activation_fn=tf.nn.relu, scope=None, reuse=None):
    ...
    with tf.variable_scope(scope, "Block35", [net], reuse=reuse):
        ...
        up    = slim.conv2d(mixed, net.get_shape()[3], 1, normalizer_fn=None,
                            activate_fn=None, scope="Conv2d_1x1")
        scaled_up=up*scale
        if activation_fn==tf.nn.relu6:
            # Use clip_by_value to simulate bandpass activation
            scale_up=tf.clip_by_value(scaled_up, -6.0, 6.0)
        net+=scaled_up
        if activation_fn:
...
def block8(net, scale=1.0, activation_fn=tf.nn.relu, scope=None, reuse=None):
    ...
    with tf.variable_scope(scope, "Block8", [net], reuse = reuse):
        with tf.variable_scope("Branch_0"):
            tower_conv = slim.conv2d(net, 182, 1, scope = "Conv2d_1x1")
        with tf.variable_scope("Branch_1"):
...
def inception_resnet_v2_base(inputs, final_endpoint="Conv2d_7b_1x1",
                             output_stride=16, align_feature_maps=False,
    ...
    """
      ...
      Raises:
        ValueError: if final_endpoint is not set to one of the predefined values,
          or if the output_stride is not 8 or 16, or if the output_stride is 8 and 16
    ...
    with tf.variable_scope(scope, "InceptionResnetV2", [inputs]):
        with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d],
            ...
            # TODO(alemi): Register intermidate endpoints
            net = slim.repeat(net, 10, block35, scale=.17,
            ...
            with tf.variable_scope("Mixed_6a"):
                ...
                with tf.variable_scope("Branch_2"):
                    tower_pool = slim.max_pool2d(net, 3, stride = 1 if use_atrous else 2,
                                                 padding = padding, scope = "MaxPool_1a_3x3")
                net = tf.concat([tower_conv, tower_conv1_2, tower_pool],3)
            if add_and_check_final("Mixed_6a", net): return net, end_points

            # TODO(alemi): register intermediate endpoints
            with slim.arg_scope([slim.conv2d], rate = 2 if use_atrous else 1):
                net = slim.repeat(net, 20, block17, scale = .10,
                                  activation_fn = activation_fn)
