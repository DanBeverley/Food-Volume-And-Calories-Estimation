...
def read_label_file(dataset_dir, filename = LABELS_FILENAME):
    ...
    Args:
        dataset_dir: directory in which the labels file is found.
...
def get_split(split_name, dataset_dir, file_pattern='foods_%s_*.tfrecord', file_pattern_for_counting='foods'):
    ...
    if split_name not in ['train', 'validation']:
        raise ValueError(
            ...
            train of validation as the split_name" % (split_name))
    ...
    keys_to_features = {
        ...
        "image/class/label":tf.FixedLenFeature([], tf.int64, default_value = tf.zeros([],
                                                                                      dtype = tf.int64))}
    # Create the items_to_handler dictionary for the decoder
    items_to_handlers = {
...
def load_batch(dataset, batch_size, height, width, is_training = True):
    ...
    # Perform the correct preprocessing for the image depending if it is training or evaluating
    image = inception_preprocessing.preprocess_image(raw_image, height, width, is_training)
    # As for the raw images, do a simple reshape to batch it up
...
def _dataset_exists(dataset_dir, _NUM_SHARDS, output_filename):
    for split_name in ['train', 'validation']:
        for shard_id in range(_NUM_SHARDS):
            filename = _get_dataset_filename(dataset_dir, split_name, shard_id, _NUM_SHARDS=_NUM_SHARDS,
...
def _get_filename_and_classes(dataset_dir, output_filename):
