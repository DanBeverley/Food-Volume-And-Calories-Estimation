"""
The code is used the evaluates our trained model against a number of datasets
"""
import functools
import argparse
import os
...
from src.dataset_utils import read_label_file

tf.logging.set_verbosity(tf.logging.INFO)


def evaluate(model_dir, dataset_dir):
    """Begin evaluating the entire architecture"""
    # Session configuration
    sess_config = tf.ConfigProts(allow_soft_placement = True,
                                 log_device_placement = True,
                                 intra_op_parallelism_threads = 0, # Auto compute how many threads to run
    ...
    classifier = tf.estimator.Estimator(
        ...
        config = config)
    # .predict() return an iterator of dicts
    y = classifier.predict(input_fn=eval_input_fn)
    num_food_image = {}

    for pred in y:
        predicted_class = labels_to_class_names[int(pred["classes"])]
        food_dir = "../Validations/%s/%s"%(os.path.basename(model_dir), predicted_class)
...
if __name__=="__main__":
    ...
    PARSER.add_argument('--model',
                        required=True,
                        help="Name of the pre-trained model\'s folder.")
