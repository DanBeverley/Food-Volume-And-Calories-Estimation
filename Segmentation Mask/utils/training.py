...
def get_labels_from_annotation(annotation_tensor, class_labels):
    """
    The function returns tensor of size (width, height, num_classes) which is derived from annotation
    tensor with sizes (width, height) where value at each position represents a class.

    The function requires a list with class values like [0,1,2,3] -- they are used to derive labels.
    Derived values will be ordered in the same way as the class numbers were provided in the list .
    Last value in the aforementioned list represents a value that indicate the pixel should be masked out
    ...
    # the ambiguous regions or regions that should not be used for training
    # TODO: properly replace class_labels list with some custom object
...
def get_labels_from_annotation_batch(annotation_batch_tensor, class_labels):
    """Returns tensor that is of a size
    (batch_size, width, height, num_classes) derived fom annotation tensor of size
    (batch_size, width, height) where value at each position represent a class
    Requires a list with class values like [0,1,2,3] -- used to derive labels.
    Derived values will be ordered in the same way as the class numbers were provided
    in the list. Last value in the aforementioned list represents a value that indicate
    the pixel should be masked out
    So, the size of num_classes := len(class_labels) -1
    ...
    """
    batch_labels = tf.map_fn(fn=lambda x: get_labels_from_annotation(annotation_tensor=x,
...
def get_valid_entries_indices_from_annotation_batch(annotation_batch_tensor, class_labels):
    ...
    Returns tensor that contains the indices of valid entries according to the annotation tensor.
    This can be used to later on extract only valid entries from logits tensor and labels tensor.
    The function supposed to work with batch input like [b,w,h] . The output is a tensor which contains
    indexes of valid entries. The function can also work witha single annotation like [w,h] --
    output will be (num_valid_entries, 2)
    Parameters
    ----------
    annotation_batch_tensor : Tensor of size (batch_size, width, height)
    ...
    # which number was used in the annotation to mask out
    # the ambiguous regions or regions that should not be used for training
    # TODO: properly replace class labels list with custom object
    mask_out_class_label = class_labels[-1]

    # Get binary mask for the pixels that we want to use for training.
    # Doing this due to some pixels are masked as ambiguous
