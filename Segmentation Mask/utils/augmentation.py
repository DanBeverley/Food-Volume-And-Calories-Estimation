...
def apply_with_random_selector(x, func, num_cases):
    ...
    Returns:
        func(x, sel) result, where func receives selector value os integer,
        but sel sampled dynamically"""
...
def distort_color(image, color_ordering=0, fast_mode=True, scope=None):
    """Distort Tensor image color.
    Each distortion is non-commutative, ordering of color ops matters.
    Randomly permute the ordering is ideally. For less complexity, select
    a distinct ordering of color ops for each preprocessing thread
    Args:
        image: 3D Tensor contains single image in [0,1]
        color_ordering: Python int, a type of distortion (valid values: 0-3)
        fast_mode: Avoid slower ops (random_hue and random_contrast)
        scope: Optional scope for name_scope.
    Returns:
        3D Tensor color-distorted image on range [0,1]
    Raises:
    ...
    with tf.name_scope(scope, "distort_color", [image]):
        ...
        return tf.clip_by_value(image, .0, 1.0)

def flip_randomly_left_right_image_with_annotation(image_tensor, annotation_tensor):
    """Accepts image tensor and annotation tensor and return randomly flipped tensors of both.
    Function performs random image flip and annotation tensors with probability of 1/2
    The flip is performed or not performed for image and annotation consistently for image matching
    Args:
    ...
    Returns:
        randomly_flipped_image: Tensor of size (width, height, 3) of type tf.float
...
def distort_randomly_image_color(image_tensor, fast_mode=False):
    ...
    for inception model training in TF-Slim (the link below in comments). All the
    ...
    # Randomly distort the color of image .4 ways to do so (as far as i know :PP)
    # Credit: TF-Slim
...
def scale_randomly_image_with_annotation_with_fixed_size_output(img_tensor, annotation_tensor,
                                                                output_shape,
                                                                min_relative_random_scale_change=.9,
                                                                max_relative_random_scale_change=1.1,
                                                                mask_out_number=255):
    """Returns tensor of a size (output_shape, output_shape, depth) and (output_shape, output_shape, 1).
       ...
       max_relative_random_scale_change : float
    ...
    rand_var = tf.random_uniform(shape=[1],
                                 ...
                                 maxval=max_relative_random_scale_change)
    ...
    resized_annotation = tf.image.resize_image_with_crop_or_pad(annotation_batched, scaled_input_shape)

    resized_img = tf.squeeze(resized_img, axis = 0)
    resized_annotation = tf.squeeze(resized_annotation, axis = 0)

    # Shift all the classes by one -- for differentiate
