# Important: Using PIL to read .png files later
# Done on purpose to read indexed png files
# in a special way -- only indexes and not map the indexes
# to actual rgb values. This is specific to PASCAL VOC
# dataset data . skimage.io.imread() is alternative for this method
...
def write_image_annotation_pairs_to_tfrecord(filename_pairs, tfrecords_filename):
    ...
    for img_path, annotation_path in filename_pairs:
        ...
        # Reason for storing image sizes was demonstrated
        # in previous example -- Sizes of images need to be known
        # to later read raw serialized string, convert to 1d array and convert to
        # respective shape the image used to have
        height = img.shape[0]
        width = img.shape[1]
        ...
        example = tf.train.Example(features = tf.train.Features(feature={
            ...
            "image_raw":_int64_feature(img_raw),
...
def read_image_annotation_pairs_from_tfrecord(tfrecords_filename):
    ...
    for string_record in record_iterator:
        ...
        img_string = (example.feature.feature['image_raw'].bytes_list.value[0])
        annotation_string = (example.features.feature["mask_raw"].bytes_list.value[0])
        img_id = np.fromstring(img_string, dtype = np.uint8)
        img = img_id.reshape((height, width, -1))

        annotation_id = np.fromstring(annotation_string, dtype=np.uint8)

        # Annotaions don't have depth (Third dimension)
        # TODO: check whether it works with other datasets

        annotation = annotation_id.reshape((height, width))
...
def read_tfrecord_and_decode_into_image_annotation_pair_tensors(tfrecord_filenames_queue):
    ...
    # TODO: See if removing third dimension is necessary
