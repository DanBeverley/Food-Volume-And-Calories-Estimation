...
def unpool(inputs, scale):
    return tf.image.resize_bilinear(inputs, size=[tf.shape(inputs)[1]*scale,
...
def ChainedResidualPooling(inputs, features = 256):
    ...
    net = slim.max_pool2d(net_relu, [5,5], stride = 1, padding="SANE")
    net = slim.conv2d(net, features, 3)
    net_sum_2 = tf.add(net, net_sum_1)
    return net_sum_2
def MultiResolutionFusion(high_inputs = None, low_inputs = None, features = 256):
    """Combine information from different resolutions to get more understanding of the input"""
...
def model(images, weight_decay = 1e-5, is_training = True):
    ...
    with slim.arg_scope(resnet_v1.resnet_arg_scope(weight_decay=weight_decay)):
        logits, end_points = resnet_v1.resnet_v1_101(images, is_training = is_training,
                                                     scope = "resnet_v1_101")
    with tf.variable_scope("feature_fusion", values = [end_points.values]):
        batch_norm_params = {"decay":.997, "epsilon":1e-5, "scale":True, "is_training":is_training}
        with slim.arg_scope([slim.conv2d],
                            activation_fn = tf.nn.relu,
                            normalizer_fn = slim.batch_norm,
                            normalizer_parms = batch_norm_params,
