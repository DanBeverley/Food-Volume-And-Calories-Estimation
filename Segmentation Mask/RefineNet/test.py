...
def main(argv = None):
    ...
    global_step = tf.get_variables("global_step", [], initializer = tf.constant_initializer(0),
                                   trainable = False)
    ...
    annotation_batch_tensor = tf.image.resize_nearest_neighbor(images = annotation_batch_tensor,
    ...
    miou, miou_update_op = tf.contrib.metrics.streaming_mean_iou(pred, gt,
    ...
    with tf.get_default_graph().as_default():
        ...
        variable_averages = tf.train.ExponentialMovingAverage(.997, global_step)
        ...
        with tf.Session(config = tf.ConfigProto(allow_soft_placement = True)) as sess:
            ...
            for i in range(1449):
                start = time.time()
                image_np, annotation_np, pred_np, tmp_acc, tmp_miou = sess.run([image, annotation, pred,
                                                                                acc_update_op,
                                                                                miou_update_op])
