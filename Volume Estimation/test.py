...
def main():
    model = define_model(is_resnet = False, is_densenet = False,
                         is_senet = True)
    model = torch.nn.DataParallel(model).cuda()
    model.load_state_dict(torch.load("./pretrained_model/model_senet"))

    test_loader = loaddata.getTestingData(1)
    test(test_loader, model, .25)
...
def test(test_loader, model, thre):
    ...
    for i, sample_batched in enumerate(test_loader):
        ...
        depth = depth.cuda(device = None, non_blocking = False)
        image = image.cuda()
        ...
        output = model(image)
        output = torch.nn.functional.upsample(output, size=[depth.size(2),
                                                            depth.size(3)], mode = "bilinear")
...
def define_model(is_resnet, is_densenet, is_senet):
    ...
    if is_densenet:
        original_model = densenet.densetnet161(pretrained=True)
